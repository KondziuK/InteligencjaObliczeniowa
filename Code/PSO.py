...
class PSO:
    ...
    def init_with_random_values(self) -> list:
        ...
        """
        particles = []
        min_x = self.min_x + self.ref_image.shape[0] / 2
        min_y = self.min_y + self.ref_image.shape[1] / 2
        max_x = self.max_x - self.ref_image.shape[0] / 2
        max_y = self.max_y - self.ref_image.shape[1] / 2
    ...
    def update_particles_position(self) -> None:
        min_x = int(self.min_x + self.ref_image.shape[0] / 2)
        min_y = int(self.min_y + self.ref_image.shape[1] / 2)
        max_x = int(self.max_x - self.ref_image.shape[0] / 2)
        max_y = int(self.max_y - self.ref_image.shape[1] / 2)
        for particle in self.particles:
            position = np.zeros(4)
            if min_x > int(particle.position[0] + particle.velocity[0]):
                print("MIN")
                position[0] = int(min_x + (min_x - particle.position[0] - particle.velocity[0]))
            elif max_x < particle.position[0] + particle.velocity[0]:
                print("MAX")
                position[0] = int(max_x - particle.velocity[0] + (max_x - particle.position[0]))
            else:
                print("NONE")
                position[0] = int(particle.position[0] + particle.velocity[0])

            if min_y > int(particle.position[1] + particle.velocity[1]):
                position[1] = int(min_y + (min_y - particle.position[1] - particle.velocity[1]))
            elif max_y < particle.position[1] + particle.velocity[1]:
                position[1] = int(max_y - particle.velocity[1] + (max_y - particle.position[1]))
            else:
                position[1] = int(particle.position[1] + particle.velocity[1])

            # if self.min_s > particle.position[2] + particle.velocity[2]:
            #     position[2] = - (particle.position[2] + particle.velocity[2])
            # elif self.max_s < particle.position[2] + particle.velocity[2]:
            #     position[2] = self.max_s - particle.velocity[2] + (self.max_s - particle.position[2])
            # else:
            #     position[2] = particle.position[2] + particle.velocity[2]
            position[2] = 1

            if self.min_r > particle.position[3] + particle.velocity[3]:
                position[3] = - (particle.position[3] + particle.velocity[3])
            elif self.max_r < particle.position[3] + particle.velocity[3]:
                position[3] = self.max_s - particle.velocity[3] + (self.max_s - particle.position[3])
            else:
                position[3] = particle.position[3] + particle.velocity[3]
            print(position)
...
def main():
    ...
    (refImage.shape[0] * pso.gbest_position[2])/2
    start_point = (int(pso.gbest_position[1] - (refImage.shape[1] * pso.gbest_position[2])/2), int(pso.gbest_position[0] - (refImage.shape[0] * pso.gbest_position[2])/2))
    end_point = (int(pso.gbest_position[1] + (refImage.shape[1] * pso.gbest_position[2])/2), int(pso.gbest_position[0] + (refImage.shape[0] * pso.gbest_position[2])/2))
    color = (68, 104, 124)
    thickness = 3
